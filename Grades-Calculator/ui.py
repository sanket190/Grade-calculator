...
class MainMenuPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.author = tk.Label(self, text='Sanket and Sai', font=HEADER_FONT, bg='light grey')
        self.author.place(relx=.39, rely=.3)

        # Start button
        self.calc_button = tk.Button(self, text='Start', font=DEFAULT_FONT,
                                     command=(lambda: self._change_frame_and_start_button_text(controller)))
...
class CalculatorPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.category_percentage_label = tk.Label(self, text='Percentage of Category', font=HEADER_FONT,
                                                  bg='light grey')
        ...
        self.category_button = tk.Button(self, text="Add new grade category", font=DEFAULT_FONT, command=(
            lambda: self._create_new_category(self.new_category.get(), self.new_category_percentage.get())))
        self.category_button.grid(row=4, column=0, pady=5)

        # Create category removal capabilities
        self.remove_category_button = tk.Button(self, text="Remove grade category", font=DEFAULT_FONT,
                                                command=(lambda: self._remove_category(self.chosen_category.get())))
        ...
        self.assignment_score_label = tk.Label(self, text='Assignment Score (Score/Possible Score)', font=HEADER_FONT,
                                               bg='light grey')
        ...
        self.assignment_button = tk.Button(self, text="Add new assignment", font=DEFAULT_FONT, command=(
            lambda: self._create_new_assignment(self.chosen_category.get(), self.new_assignment.get(),
        ...
        self.back_button = tk.Button(self, text='Back', font=DEFAULT_FONT,
                                     command=(lambda: controller.show_frame(MainMenuPage)))
        self.back_button.grid(row=4, column=1, sticky='w', padx=10)
        ...
        self.reset_button.grid(row=5, column=1, sticky='w', padx=10)

        # Create a button that calculates the grade and creates a pop up
        self.calculate_grade_button = tk.Button(self, text='Calculate grade', font=DEFAULT_FONT,
                                                command=self._calculate_current_grade)
        self.calculate_grade_button.grid(row=3, column=1, stick='e', pady=10, padx=10)
    ...
    def _create_new_category(self, category_name, category_percentage):
        ...
        try:
            ...
            self.category_chooser['menu'].add_command(label=category_name,
                                                      command=(lambda: self.chosen_category.set(category_name)))
    ...
    def _create_new_assignment(self, category_name, assignment_name, score):
        ...
        if not self.can_add_assignments:
            tkinter.messagebox.showerror(title='Cannot add assignment',
                                         message='There are no categories added, so assignments cannot be added yet')
    ...
    def _reset_calculator(self):
        ...
        # Clear the entries
        for entry in (self.assignment_name_entry, self.assignment_score_entry, self.category_name_entry,
                      self.category_percentage_entry):
            entry.delete(0, 'end')
    ...
    def _calculate_current_grade(self):
        ...
        if len(self.calculator.categories) > 0:
            self.can_calculate_grade = all(
                [len(v[GradeCalculator.ASSIGNMENTS]) > 0 for v in self.calculator.categories.values()])

        if not self.can_calculate_grade:
            tkinter.messagebox.showerror(title='Cannot calculate grade yet',
                                         message='A category must exist before a grade can be calculated. All categories must have at least one assignment before a grade can be calculated.')
            return

        tkinter.messagebox.showinfo(title='Grade Calculator - Calculated Grade',
                                    message=f'Calculated grade for class: {self.calculator.calculate_total_grade() * 100}%')
